...
<?php if (validation_errors()) :  ?>
  ...
  <a href="<?= base_url('pariwisata/tambah_edukasi') ?>" class="btn btn-primary mb-3" ><i class="fa fa-plus"></i> Tambah</a>
    <a href="<?= base_url('Cetak/cetak_edukasi') ?>" class="btn btn-success mb-3" target="blank" target="_blank" ><i class="fas fa-print"></i> Print</a>

  <!-- DataTales -->
  <div class="card shadow mb-20">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary ">Data Wisata Edukasi</h6>
    </div>
    <div class="card-body">
         ...
         <thead>
          ...
          <tbody>
            ...
            <?php foreach ($edukasi as $bdy) : ?>
              <tr>
                <td scope=" row"><?= $i; ?></td>
                <td><?= $bdy['objek_wisata']; ?></td>
                <td><?= $bdy['daerah']; ?></td>
                <td><?= $bdy['lokasi']; ?></td>
                <td><?= $bdy['jarak_tempuh']; ?></td>
                <td>
                    <img src="<?= base_url('assets/img/profile/') . $bdy['gambar']; ?>"  width="100" height="100">
                </td>
                <td>
                <a class="btn btn-info btn-sm" href="" data-id="<?php echo $bdy['id'] ?>" data-objek_wisata="<?php echo $bdy['objek_wisata'] ?>" data-daerah="<?php echo $bdy['daerah'] ?>" data-lokasi="<?php echo $bdy['lokasi'] ?>"data-jarak_tempuh="<?php echo $bdy['jarak_tempuh'] ?>" data-deskripsi="<?php echo $bdy['deskripsi'] ?>" data-gambar="<?php echo $bdy['gambar'] ?>" data-toggle="modal" data-target="#edit-data"></i>Edit</a>
               <a href=" <?= base_url('pariwisata/detail_edukasi/') . $bdy['id']; ?>" class="btn btn-warning btn-sm">Detail</a>
                   <a href="<?= base_url('pariwisata/hapus_edukasi/' . $bdy['id']) ?>" onclick="return confirm('Anda Yakin ingin Menghapus Data Ini???');" class="btn btn-danger btn-sm"> Hapus</a>
              </td>
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title" id="myModalLabel" style="font-weight: bold;">Edit Data Wisata Edukasi</h3>
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true" style="font-weight: bold;">x</button>
      </div>
      <form class="form-horizontal" action="<?php echo base_url('Pariwisata/ubah_edukasi') ?>" method="post" enctype="multipart/form-data" role="form">
