...
<?php if (validation_errors()) :  ?>
  ...
  <a href="<?= base_url('pariwisata/tambah_kuliner') ?>" class="btn btn-primary mb-3" ><i class="fa fa-plus"></i> Tambah</a>
    <a href="<?= base_url('Cetak/cetak_kuliner') ?>" class="btn btn-success mb-3" target="blank" target="_blank" ><i class="fas fa-print"></i> Print</a>

  <!-- DataTales -->
  <div class="card shadow mb-20">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary ">Data Wisata Kuliner</h6>
    </div>
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-hover display nowrap dataTable dtr-inline table table-bordered table-striped text-center" id="dataTable" width="100%" cellspacing="0" style="text-align:center">
         ...
         <thead>
          ...
          <tbody>
            <?php $i = 1; ?>
            <?php foreach ($kuliner as $kln) : ?>
              <tr>
                <td scope=" row"><?= $i; ?></td>
                <td><?= $kln['objek_wisata']; ?></td>
                <td><?= $kln['daerah']; ?></td>
                <td><?= $kln['lokasi']; ?></td>
                <td><?= $kln['jarak_tempuh']; ?></td>

                <td>
                    <img src="<?= base_url('assets/img/profile/') . $kln['gambar']; ?>"  width="100" height="100">
                </td>
                <td>
                 <a class="btn btn-info btn-sm" href="" data-id="<?php echo $kln['id'] ?>" data-objek_wisata="<?php echo $kln['objek_wisata'] ?>" data-daerah="<?php echo $kln['daerah'] ?>" data-lokasi="<?php echo $kln['lokasi'] ?>"data-jarak_tempuh="<?php echo $kln['jarak_tempuh'] ?>" data-deskripsi="<?php echo $kln['deskripsi'] ?>" data-gambar="<?php echo $kln['gambar'] ?>" data-toggle="modal" data-target="#edit-data"></i>Edit</a>
               <a href=" <?= base_url('pariwisata/detail_kuliner/') . $kln['id']; ?>" class="btn btn-warning btn-sm">Detail</a>
               <a href="<?= base_url('pariwisata/hapus_kuliner/' . $kln['id']) ?>" onclick="return confirm('Anda Yakin ingin Menghapus Data Ini???');" class="btn btn-danger btn-sm"> Hapus</a>
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title" id="myModalLabel" style="font-weight: bold;">Edit Data Wisata Kuliner</h3>
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true" style="font-weight: bold;">x</button>
      </div>
      <form class="form-horizontal" action="<?php echo base_url('Pariwisata/ubah_kuliner') ?>" method="post" enctype="multipart/form-data" role="form">
